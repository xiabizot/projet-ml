        ...
        if reco:
            ...
            if r_cur.get('track_id'):
                ...
                if ap_c.exists():
                    ...
                    with col_pl:
                        st.caption(f'Piste seed : {r_cur.get("title", "?")} -- {r_cur.get("artist", "?")}')
            ...
            for rank, rec in enumerate(reco, 1):
                ...
                with col_bar:
                    st.markdown(f'<div style="display:flex; align-items:center; gap:8px; padding-top:8px">'
                                ...
                                unsafe_allow_html=True)
                with col_btn:
                    rec_tid = int(rec['track_id'])
                    rec_folder = f'{rec_tid:06d}'[:3]
                    rec_audio = Path(__file__).parent / 'data' / 'raw' / 'fma_small' / 'fma_small' / rec_folder / f'{rec_tid:06d}.mp3'
                    if rec_audio.exists():
                        with open(rec_audio, 'rb') as af_r:
                            st.audio(af_r.read(), format='audio/mp3')
            # Comparatif Claude seed vs reco
            st.markdown('')
            st.markdown('<div class="section-title">Comparatif acoustique (Claude)</div>',
                        unsafe_allow_html=True)
            st.markdown('<div style="font-size:0.75rem; color:#aaa; text-align:center; margin-bottom:8px">'
                        'Comparez l\'analyse acoustique de la piste source avec une recommandation'
                        '</div>', unsafe_allow_html=True)

            reco_names = [f'{rec["title"]} -- {rec["artist"]}' for rec in reco]
            selected_reco = st.selectbox('Comparer avec', reco_names, index=None,
                                         placeholder='Choisissez une recommandation', key='reco_compare')
            _, col_btn_c, _ = st.columns([2, 2, 2])
            with col_btn_c:
                compare_btn = st.button('Comparer avec Claude', key='compare_claude')

            if selected_reco and compare_btn:
                reco_idx = reco_names.index(selected_reco)
                rec_sel = reco[reco_idx]
                with st.spinner('Analyse comparative en cours...'):
                    r_seed = st.session_state.get('seed_originale', st.session_state.get('r2', st.session_state['r1']))
                    prompt = (
                        f'Tu es un expert en acoustique musicale.\n\n'
                        f'Piste source : {r_seed.get("title","?")} de {r_seed.get("artist","?")} '
                        f'- genre {r_seed.get("pred_genre","?")}\n'
                        f'Piste recommandee (similarite {rec_sel["similarity"]}%) : '
                        f'{rec_sel["title"]} de {rec_sel["artist"]} - genre {rec_sel["genre"]}\n\n'
                        f'En 4-5 phrases, explique ce qui rapproche acoustiquement ces deux morceaux '
                        f'(texture, timbre, rythme, energie, instrumentation) et ce qui les distingue. '
                        f'Reste sobre et factuel.'
                    )
                    try:
                        expl = explain_with_claude(prompt_override=prompt)
                        import re as re_mod
                        expl_c = re_mod.sub(r'[*#_`]', '', expl)
                        col_s, col_r = st.columns(2)
                        with col_s:
                            st.markdown(f'**{r_seed.get("title","?")}**')
                            st.caption(f'{r_seed.get("artist","?")} -- {r_seed.get("pred_genre","?")}')
                        with col_r:
                            st.markdown(f'**{rec_sel["title"]}**')
                            st.caption(f'{rec_sel["artist"]} -- {rec_sel["genre"]} ({rec_sel["similarity"]}%)')
                        st.markdown(f"""<div style="
                            background: linear-gradient(135deg, #1a1a2e 0%, #16213e 100%);
                            border-radius: 10px; padding: 16px; margin-top: 8px;
                            border-left: 3px solid #4a9eff;
                            font-size: 0.85rem; color: #ccc; line-height: 1.7;
                        ">{expl_c}</div>""", unsafe_allow_html=True)
                    except Exception as e:
                        st.caption(f'Claude API non disponible : {e}')

                    # Grad-CAM comparatif
                    try:
                        import torch, torch.nn as nn, torch.nn.functional as F
                        import matplotlib.pyplot as plt, matplotlib.cm as cm_mpl, cv2
                        from src.agent import AudioCNN

                        DEVICE_GC = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
                        model_path = Path(__file__).parent / 'outputs' / 'cnn' / 'best_audio_cnn.pth'
                        spec_dir = Path(__file__).parent / 'spectrogrammes'
                        seed_tid = r_seed.get('track_id')
                        reco_tid = rec_sel['track_id']
                        sp_seed = spec_dir / f'{int(seed_tid):06d}.npy' if seed_tid else None
                        sp_reco = spec_dir / f'{int(reco_tid):06d}.npy'

                        if sp_seed and sp_seed.exists() and sp_reco.exists() and model_path.exists():
                            cnn = AudioCNN(num_classes=8).to(DEVICE_GC)
                            cnn.load_state_dict(torch.load(model_path, map_location=DEVICE_GC))
                            cnn.eval()

                            class GradCAMR:
                                def __init__(s, model, layer):
                                    s.model=model; s.g=None; s.a=None
                                ...
                                def gen(s, t):
                                    s.model.zero_grad(); out=s.model(t); tc=out.argmax(1).item()
                                    oh=torch.zeros_like(out); oh[0,tc]=1.0; out.backward(gradient=oh)
                                    w=s.g.mean(dim=(2,3),keepdim=True)
                                    c=F.relu((w*s.a).sum(dim=1).squeeze())
                                    mn,mx=c.min(),c.max()
                                    if mx-mn>1e-8: c=(c-mn)/(mx-mn)
                                    return c.cpu().numpy()

                            gc = GradCAMR(cnn, cnn.conv4)
                            TF = 1291

                            def ld(p):
                                s=np.load(p)
                                s=s[:,:TF] if s.shape[1]>=TF else np.pad(s,((0,0),(0,TF-s.shape[1])))
                                return (s-s.mean())/(s.std()+1e-8)

                            ss, sr = ld(sp_seed), ld(sp_reco)
                            cs = gc.gen(torch.tensor(ss,dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(DEVICE_GC))
                            cr = gc.gen(torch.tensor(sr,dtype=torch.float32).unsqueeze(0).unsqueeze(0).to(DEVICE_GC))
                            cs_r = cv2.resize(cs,(TF,128)); cr_r = cv2.resize(cr,(TF,128))
                            sn = lambda x: (x-x.min())/(x.max()-x.min()+1e-8)
                            ov_s = np.clip(0.55*cm_mpl.magma(sn(ss))[:,:,:3]+0.45*cm_mpl.jet(cs_r)[:,:,:3],0,1)
                            ov_r = np.clip(0.55*cm_mpl.magma(sn(sr))[:,:,:3]+0.45*cm_mpl.jet(cr_r)[:,:,:3],0,1)

                            st.markdown('')
                            st.markdown('<div class="section-title">Grad-CAM comparatif</div>', unsafe_allow_html=True)
                            st.caption('Zones chaudes = decision du CNN. Si les zones se ressemblent, le cosinus est eleve.')
                            fig, axes = plt.subplots(1,2,figsize=(12,3))
                            axes[0].imshow(ov_s, aspect='auto', origin='lower', extent=[0,30,0,128])
                            axes[0].set_title(r_seed.get('title','Seed')[:30], fontsize=8, color='#ccc')
                            axes[1].imshow(ov_r, aspect='auto', origin='lower', extent=[0,30,0,128])
                            axes[1].set_title(rec_sel['title'][:30], fontsize=8, color='#ccc')
                            for ax in axes:
                                ax.tick_params(labelsize=5, colors='#888')
